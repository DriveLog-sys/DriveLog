...
const hasFreshCache = cacheAge < 300000 && !!localStorage.getItem('dl_posts_cache');
if (!_sbOk()) { _initSb(); }
if (!_sbOk()) return;
...
try { if (!_prefetchSessionPromise && !_prefetchPostsPromise) { _initSb(); startPrefetch(); } } catch(_) {}
...
const profilesStale = profileCacheAge > 10 * 60 * 1000;
...
if (from === S.user.username) return;
...
function renderTicker() { renderHotPanel(); }
let _hotSliderIdx = 0;
let _hotSliderAuto = null;
const HOT_SLIDE_DURATION = 8000;
...
client_id: '575055658940-placeholder.apps.googleusercontent.com',
...
const hpMax = hps.length ? Math.ceil(Math.max(...hps) / 50) * 50 : 500;
